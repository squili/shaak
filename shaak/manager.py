...
class Manager(commands.Cog):
    ...
    async def load_module(self, cls):
        ...
        await self.bot.add_cog(loaded_cog)
    ...
    async def max_guilds_check(self, guild: discord.Guild):
        if guild.member_count <= 60:
            await guild.leave()
            embed = Embed(
                description='Shaak is currently not allowed in server with 60 or less members. You can join [the support server](https://discord.gg/SWMKshyutT) to learn when this restriction will be lifted.')
            embed.set_footer(
                text=f'You are receiving this message because you own the server {guild.name}')
    ...
    async def settings_set(self, ctx: commands.Context, setting_name: str, *, setting_value: Optional[str]):
        ...
        if setting_name in setting_structure:
            if setting_value != None:
                setting_value = setting_structure[setting_name][0](
                    setting_value)
            if setting_name == 'error_channel':
                self.utils.ensure_guild_contains_channel(
                    ctx.guild.id, setting_value)
    ...
    async def settings_list(self, ctx: commands.Context):
        ...
        for name, converter in setting_structure.items():
            ...
            formatted.append(
                f'{name}: {"unset" if value == None else converter[1](value)}')
